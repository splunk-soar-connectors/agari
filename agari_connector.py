...
class AgariConnector(BaseConnector):
    ...
    def _validate_integer(self, action_result, parameter, key, allow_zero=False):
        ...
        if parameter is not None:
            try:
                if not float(parameter).is_integer():
                    return action_result.set_status(
                        phantom.APP_ERROR, AGARI_VALID_INT_MSG.format(param=key)
                    ), None
            ...
            if parameter < 0:
                return action_result.set_status(
                    phantom.APP_ERROR, AGARI_NON_NEG_INT_MSG.format(param=key)
                ), None
            if not allow_zero and parameter == 0:
                return action_result.set_status(
                    phantom.APP_ERROR, AGARI_NON_NEG_NON_ZERO_INT_MSG.format(param=key)
                ), None
    ...
    def _filter_comma_seperated_fields(self, action_result, field, key):
        ...
        if field:
            ...
            if not fields_list:
                return action_result.set_status(
                    phantom.APP_ERROR, AGARI_ERR_INVALID_FIELDS.format(field=key)
                ), None
    ...
    def _paginator(self, action_result, params, endpoint, key, offset, max_results):
        ...
        while True:
            ret_val, items = self._make_rest_call_helper(
                action_result, endpoint, headers=self._headers, params=params
            )
    ...
    def _validate_paginator_parameters(self, action_result, param):
        ...
        ret_val, offset = self._validate_integer(
            action_result, param.get("offset", 0), "offset", True
        )
        if phantom.is_fail(ret_val):
        ...
        ret_val, max_results = self._validate_integer(
            action_result,
            param.get("max_results", AGARI_DEFAULT_MAX_RESULTS),
            "max_results",
        )
        if phantom.is_fail(ret_val):
            return action_result.get_status(), None, None
    ...
    def _parse_datetime(self, action_result, date, key):
        ...
        try:
            ...
            return action_result.set_status(
                phantom.APP_ERROR, AGARI_ERR_PARSE_DATE.format(param=key)
            ), None

        # Check date is within the max range
        if date_time_object.utcoffset():
            date_time_object = (
                date_time_object.replace(tzinfo=pytz.UTC) - date_time_object.utcoffset()
            )
        else:
            date_time_object = date_time_object.replace(tzinfo=pytz.UTC)

        if self.get_action_identifier() == "on_poll" and date_time_object < (
            self._current_utc_time - timedelta(days=AGARI_MAX_DAYS)
        ).replace(tzinfo=pytz.UTC):
            return action_result.set_status(
                phantom.APP_ERROR, AGARI_ERR_DATE_NOT_IN_RANGE.format(key=key)
            ), None
    ...
    def _validate_sort_parameter(self, action_result, sort, key):
        ...
        if sort:
            try:
                ...
                sort_list = [
                    value.strip() for value in sort.split(",") if value.strip()
                ]

                # No element found, return error
                if not sort_list:
                    return action_result.set_status(
                        phantom.APP_ERROR, AGARI_ERR_INVALID_SORT.format(param=key)
                    ), None
                ...
                for element in sort_list:
                    ...
                    if sort_type not in AGARI_SORT_VALUE_LIST:
                        return action_result.set_status(
                            phantom.APP_ERROR, AGARI_ERR_INVALID_SORT_TYPE
                        ), None
    ...
    def _validate_value_list_parameter(self, action_result, parameter, key, value_list):
        ...
        if parameter:
            ...
            if parameter not in value_list:
                ...
                return action_result.set_status(
                    phantom.APP_ERROR,
                    AGARI_ERR_INVALID_VALUE_LIST_PARAMETER.format(
                        param=key, value_list=value_list_string
                    ),
                ), None
            return phantom.APP_SUCCESS, parameter
        return phantom.APP_SUCCESS, parameter
    ...
    def _validate_datetime_parameters(self, action_result, start_date, end_date):
        ...
        # Parse start date
        if start_date:
            ret_val, start_date_object = self._parse_datetime(
                action_result, start_date, "start_date"
            )
            if phantom.is_fail(ret_val):
                return action_result.get_status(), None, None
        ...
        # Parse end date
        if end_date:
            ret_val, end_date_object = self._parse_datetime(
                action_result, end_date, "end_date"
            )
            if phantom.is_fail(ret_val):
                return action_result.get_status(), None, None
        ...
        if (
            start_date_object
            and end_date_object
            and end_date_object < start_date_object
        ):
            return (
                action_result.set_status(
                    phantom.APP_ERROR, AGARI_ERR_END_DATE_LESS_THAN_START_DATE
                ),
                None,
                None,
            )
    ...
    def _validate_fields_parameters(self, action_result, param):
        ...
        """
        ret_val, fields = self._filter_comma_seperated_fields(
            action_result, param.get("fields"), "fields"
        )
        if phantom.is_fail(ret_val):
            return action_result.get_status(), None, None, None

        ret_val, rem_fields = self._filter_comma_seperated_fields(
            action_result, param.get("rem_fields"), "rem_fields"
        )
        if phantom.is_fail(ret_val):
            return action_result.get_status(), None, None, None

        ret_val, add_fields = self._filter_comma_seperated_fields(
            action_result, param.get("add_fields"), "add_fields"
        )
        if phantom.is_fail(ret_val):
            return action_result.get_status(), None, None, None
    ...
    def _validate_list_policy_events_parameter(self, action_result, param):
        ...
        ret_val, start_date, end_date = self._validate_datetime_parameters(
            action_result, param.get("start_date"), param.get("end_date")
        )
        if phantom.is_fail(ret_val):
            return action_result.get_status(), {}

        # Validate paginator parameters
        ret_val, offset, max_results = self._validate_paginator_parameters(
            action_result, param
        )
        if phantom.is_fail(ret_val):
            return action_result.get_status(), {}
        ...
        ret_val, sort = self._validate_sort_parameter(
            action_result, param.get("sort"), "sort"
        )
        if phantom.is_fail(ret_val):
            return action_result.get_status(), {}

        # Value list validation for 'policy_action'
        ret_val, policy_action = self._validate_value_list_parameter(
            action_result,
            param.get("policy_action"),
            "policy_action",
            AGARI_POLICY_ACTION_VALUE_LIST,
        )
        if phantom.is_fail(ret_val):
            return action_result.get_status(), {}
        ...
        ret_val, exclude_alert_types = self._validate_value_list_parameter(
            action_result,
            param.get("exclude_alert_types"),
            "exclude_alert_types",
            AGARI_EXCLUDE_ALERT_TYPES_VALUE_LIST,
        )
        if phantom.is_fail(ret_val):
            return action_result.get_status(), {}
        ...
        ret_val, policy_enabled = self._validate_value_list_parameter(
            action_result,
            param.get("policy_enabled"),
            "policy_enabled",
            AGARI_POLICY_ENABLED_VALUE_LIST,
        )
    ...
    def _process_empty_response(self, response, action_result):
        ...
        return RetVal(
            action_result.set_status(
                phantom.APP_ERROR,
                AGARI_ERR_EMPTY_RESPONSE.format(code=response.status_code),
            ),
            None,
        )
    ...
    def _process_json_response(self, r, action_result):
        ...
        try:
            ...
            return RetVal(
                action_result.set_status(
                    phantom.APP_ERROR,
                    AGARI_ERR_UNABLE_TO_PARSE_JSON_RESPONSE.format(error=error_msg),
                ),
                None,
            )
    ...
    def requests_retry_session(
        self, retries, backoff_factor, status_forcelist=[429], session=None
    ):
        """
    ...
    def _make_rest_call(
        ...
        try:
            ...
            return RetVal(
                action_result.set_status(
                    phantom.APP_ERROR, "Invalid method: {}".format(method)
                ),
                resp_json,
            )
        ...
        try:
            r = request_func(
                url, verify=True, json=json, data=data, headers=headers, params=params
            )
        except Exception as e:
            error_msg = self._get_error_message_from_exception(e)
            return RetVal(
                action_result.set_status(
                    phantom.APP_ERROR,
                    AGARI_ERR_CONNECTING_TO_SERVER.format(error=error_msg),
                ),
                resp_json,
            )
    ...
    def _make_rest_call_helper(
        ...
        if not token.get(AGARI_OAUTH_ACCESS_TOKEN_STRING):
            ...
            headers.update(
                {
                    "Authorization": AGARI_AUTHORIZATION_HEADER.format(
        ...
        if msg and ("403" in msg or "401" in msg):
            self.debug_print(
                "Refreshing Agari API and re-trying request to [{}] because API token was expired or invalid with error code [{}]".format(
                    url, msg
                )
            )
            ret_val = self._generate_access_token(action_result)
    ...
    def _handle_test_connectivity(self, param):
        ...
        ret_val, _ = self._make_rest_call(
            url, action_result, params=param, headers=self._headers
        )
    ...
    def _handle_get_policy_event(self, param):
        ...
        """
        self.save_progress(
            AGARI_ACTION_HANDLER_MSG.format(identifier=self.get_action_identifier())
        )
        action_result = self.add_action_result(ActionResult(dict(param)))

        ret_val, policy_event_id = self._validate_integer(
            action_result, param["id"], "id", True
        )
        if phantom.is_fail(ret_val):
            return action_result.get_status()
        ...
        params = {"fields": fields, "rem_fields": rem_fields, "add_fields": add_fields}
        ...
        return action_result.set_status(
            phantom.APP_SUCCESS, AGARI_SUCC_GET_POLICY_EVENT
        )
    ...
    def _handle_remediate_message(self, param):
        ...
        return action_result.set_status(
            phantom.APP_SUCCESS, AGARI_SUCC_REMEDIATE_MESSAGE
        )
    ...
    def _handle_list_policy_events(self, param):
        ...
        ret_val, updated_params = self._validate_list_policy_events_parameter(
            action_result, param
        )
        if phantom.is_fail(ret_val):
            return action_result.get_status()
        ...
        ret_val, alert_events = self._paginator(
            action_result,
            updated_params,
            AGARI_LIST_POLICY_EVENTS_ENDPOINT,
            "alert_events",
            updated_params["offset"],
            max_results,
        )
    ...
    def _handle_list_messages(self, param):
        ...
        ret_val, updated_params = self._validate_list_messages_parameter(
            action_result, param
        )
        if phantom.is_fail(ret_val):
            return action_result.get_status()
        ...
        if not self._is_poll_now:
            ...
            if last_fetched_id:
                if not filter_to_apply:
                    ...
                    filter_to_apply = "{} and id.gt({})".format(
                        filter_to_apply, last_fetched_id
                    )
    ...
    def _validate_ingestion_params(self, action_result, config):
        ...
        ret_val, add_fields = self._filter_comma_seperated_fields(
            action_result, config.get("add_fields"), "Fields to add"
        )
        if phantom.is_fail(ret_val):
            return action_result.get_status(), {}
        ...
        sort = (
            ...
            if config["sort"] == "oldest_first"
            else "created_at DESC, id ASC"
        )

        # Initialize 'start_date' based on type of polling
        start_date_default = str(
            self._current_utc_time - timedelta(days=AGARI_DEFAULT_DAYS)
        )
        if self._is_poll_now:
            start_date = config.get("start_date", start_date_default)
        else:
            start_date = self._state.get(
                AGARI_LAST_INGESTED_POLICY_EVENT_DATE,
                config.get("start_date", start_date_default),
            )
        ...
        if phantom.is_fail(ret_val):
            ...
            if "end_date" in action_result_message:
                end_date_message = "The 'end_date' for the On Poll action is current UTC time ({}).".format(
                    self._current_utc_time.strftime(AGARI_API_SUPPORT_DATE_FORMAT)
                )
        ...
        ret_val, exclude_alert_types = self._validate_value_list_parameter(
            action_result,
            config.get("exclude_alert_types"),
            "exclude_alert_types",
            AGARI_EXCLUDE_ALERT_TYPES_VALUE_LIST,
        )
        if phantom.is_fail(ret_val):
        ...
        if cef_mapping:
            try:
                ...
                return action_result.set_status(
                    phantom.APP_ERROR, AGARI_ERR_INVALID_JSON
                ), {}
    ...
    def _get_results_helper(self, alert_event, action_result, message_params):
        ...
        try:
            ...
            if phantom.is_fail(ret_val):
                ...
                self.debug_print(
                    "Failed to fetch policy event ID {}. {}".format(
                        policy_event_id, error_msg
                    )
                )
            ...
            if not message_id:
                self.debug_print(
                    "Failed to fetch message ID as message ID was not found"
                )
                self.debug_print(AGARI_ERR_SKIP_POLICY_EVENT.format(policy_event_id))
                self.save_progress(AGARI_ERR_SKIP_POLICY_EVENT.format(policy_event_id))
                return {}
            ...
            if phantom.is_fail(ret_val):
                error_msg = action_result.get_message()
                self.debug_print(
                    "Failed to fetch message ID {}. {}".format(message_id, error_msg)
                )
                self.debug_print(AGARI_ERR_SKIP_POLICY_EVENT.format(policy_event_id))
                self.save_progress(AGARI_ERR_SKIP_POLICY_EVENT.format(policy_event_id))
                return {}
            ...
            policy_event_updated["conditions"] = policy_event.get(
                "alert_event", {}
            ).get("conditions")
    ...
    def _get_results(self, action_result, updated_params, max_results):
        ...
        if updated_params.get("add_fields"):
            ...
            message_params = {"add_fields": add_fields}
        ...
        for alert_event in alert_events:
            ...
            try:
                ...
                message_id = policy_event.get("alert_event", {}).get(
                    "collector_message_id"
                )
                if not message_id:
                    non_ingested_policy_ids.append(str(policy_event_id))
                    continue
            ...
            return action_result.set_status(
                phantom.APP_ERROR,
                AGARI_ERR_NO_POLICY_EVENT_INGESTED.format(
                    ", ID: ".join(non_ingested_policy_ids)
                ),
            ), []
        ...
        with ThreadPoolExecutor(max_workers=self._max_workers) as executor:
            fetched_data = executor.map(
                self._get_results_helper,
                alert_events,
                action_result_list,
                message_params_list,
            )
    ...
    def _save_results(
        self, action_result, results, update_state_after, cef_mapping, sort
    ):
        """
        ...
        for data in results:
            ...
            severity = self._get_severity_from_message_trust_score(
                message.get("message_trust_score", "0")
            )
            container = self._get_container_data(policy_event, severity)
            message_artifact = self._get_message_artifact_data(
                message, cef_mapping, severity
            )
            ...
            if phantom.is_fail(status):
                self.debug_print(
                    "Error occurred while saving the container: ID {}: {}".format(
            ...
            status, message, _ = self.save_artifacts(
                [policy_event_artifact, message_artifact]
            )
            if phantom.is_fail(status):
                self.debug_print(
                    "Error occurred while saving the artifact: {}".format(message)
                )
                continue

            # Update state if the required number of container are ingested as per 'update_state_after'
            if (
                count == update_state_after
                and not self._is_poll_now
                and sort == "oldest_first"
            ):
                self._state[AGARI_LAST_INGESTED_POLICY_EVENT_DATE] = policy_event.get(
                    "created_at"
                )
            ...
            self.save_progress(
                "Policy event ID ({}) is ingested in container ID ({})".format(
                    policy_event_id, container_id
                )
            )
            self.debug_print(
                "Policy event ID ({}) is ingested in container ID ({})".format(
        ...
        if not self._is_poll_now:
            ...
            if results:
                self._state[AGARI_LAST_INGESTED_POLICY_EVENT_DATE] = (
                    results[index].get("policy_event", {}).get("created_at")
                )
                self._state[AGARI_LAST_INGESTED_POLICY_EVENT_ID] = (
                    results[index].get("policy_event", {}).get("id")
                )
                self.save_state(self._state)
                self.debug_print(AGARI_INGESTION_STATUS_UPDATED)
    ...
    def _handle_on_poll(self, param):
        ...
        if self._is_poll_now:
            max_results = param.get(
                "container_count", config.get("max_results", AGARI_DEFAULT_MAX_RESULTS)
            )
        else:
            max_results = config.get("max_results", AGARI_DEFAULT_MAX_RESULTS)
        # Integer validation for 'max_results' parameter
        ret_val, max_results = self._validate_integer(
            action_result, max_results, "max_results"
        )
        if phantom.is_fail(ret_val):
            return action_result.get_status()
        ...
        ret_val = self._save_results(
            action_result, results, update_state_after, cef_mapping, config["sort"]
        )
        if phantom.is_fail(ret_val):
    ...
    def initialize(self):
        ...
        ret_val, self._max_workers = self._validate_integer(
            self,
            config.get("max_workers_for_polling", AGARI_DEFAULT_MAX_WORKERS),
            "max workers for polling",
        )
        if phantom.is_fail(ret_val):
        ...
        self._headers = {
            ...
            "User-Agent": AGARI_USER_AGENT.format(
        ...
        try:
            self._session = self.requests_retry_session(
                retries=AGARI_DEFAULT_NUM_RETRIES,
                backoff_factor=AGARI_DEFAULT_BACKOFF_FACTOR,
            )
        except Exception:
